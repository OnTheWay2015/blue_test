...
int CMySQLConnection::GetResults(IDBRecordSet * pDBRecordset)
{
	...
	//pDBRecordset->Destory();//todo
	if(m_UseSTMTMode)
	{
		//if(pDBRecordset->IsKindOf(GET_CLASS_INFO(CMySQLDynamicRecordSet))) //todo
		{
	...
	{
		//if(pDBRecordset->IsKindOf(GET_CLASS_INFO(CDBStaticRecordSet))) //todo
		{
			return FetchStaticResult((CDBStaticRecordSet *)pDBRecordset);
		}
		//else if(pDBRecordset->IsKindOf(GET_CLASS_INFO(CMySQLRecordSet)))//todo
		{
...
DWORD CMySQLConnection::FetchConnectFlags(LPCSTR FlagsStr)
{
    ...
    Stringsplit(STD_STR(FlagsStr), _T('|'), Splitter);

	//CStringSplitter Splitter(FlagsStr,'|');
	for(int i=0;i<(int)Splitter.size();i++)
	{
		CEasyString Flag=Splitter[i].c_str();
		Flag.Trim(' ');
