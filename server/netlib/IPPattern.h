		...
		if(Pattern)
		{
			if (_tcschr(Pattern,'.'))
            {
				...
				if (Split.size() >= 4)
				{
					for (int i = 0; i < 4; i++)
					{
						if (_tcschr(Split[i].c_str(), '*') != NULL)
							m_Pattern[i] = -1;
						else
							m_Pattern[i] = _ttoi(Split[i].c_str());
					}
			...
			else if (_tcschr(Pattern, ':'))
			{
				...
				if (Split.size() >= 2 && Split.size()<=8)
				{
					...
					for (UINT i = 0; i < Split.size(); i++)
					{
						if (Split[i][0] == 0)
						{
							for (UINT j = 0; j <= 8 - Split.size(); j++)
							{
								m_Pattern[p + j] = 0;
							}							
							p += 8 - (int)Split.size();
						}							
						else if (_tcschr(Split[i].c_str(), '*') != NULL)
							m_Pattern[p] = -1;
						else
							m_Pattern[p] = htons((WORD)_tcstol(Split[i].c_str(), NULL, 16));
						p++;
					}
				}
				else
