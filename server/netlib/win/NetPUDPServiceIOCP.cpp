...
bool CNetPUDPService::OnIOCPEvent(int EventID, COverLappedObject * pOverLappedObjectV)
{
...
void CNetPUDPService::Destory()
{
	...
	if (GetServer())
	{
		GetServer()->ReleaseOverLappedObject(GetID());
...
CSmartPtr<COverLappedObject>  CNetPUDPService::AllocOverLappedObject()
{
	auto pObject = m_pServer->AllocOverLappedObject(GetID());
	pObject->SetStatus(OVERLAPPED_OBJECT_STATUS_USING);
	return pObject ;
}
bool CNetPUDPService::ReleaseOverLappedObject(CSmartPtr<COverLappedObject> pObject)
{
