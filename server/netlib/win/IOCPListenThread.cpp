...
BOOL CIOCPListenThread::OnRun()
{
	...
	if(AcceptSocket==INVALID_SOCKET)
	{
		...
		{
			auto pOverLappedObject = m_pServer->AllocOverLappedObject(IO_ACCEPT2);
			if(pOverLappedObject)
			{
				m_pServer->OnIOCPEvent(IOE_ERROR,pOverLappedObject.get());
				return FALSE;
			}
			else
			{
	...
	else
	{
		...
		auto pOverLappedObject = m_pServer->AllocOverLappedObject(IO_ACCEPT2);
			if(pOverLappedObject)
			{
				...
				m_pServer->OnIOCPEvent(IOE_PACKET,pOverLappedObject.get());
				return TRUE;
			}
			else
			{
