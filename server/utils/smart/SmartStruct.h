...
public:
	...
	public:
		...
		iterator(const BYTE* pData, UINT m_DataLen)
		{
			if(m_DataLen)
			{
				m_pData = pData;
		...
		{
			m_pData = itr.m_pData;
			m_DataLen = itr.m_DataLen;
		}
		CSmartValue operator*()
		{
			CSmartValue Value;
			Value.Attach((void *)m_pData, m_DataLen, CSmartValue::VT_UNKNOWN);
			return Value;			
		}
		iterator& operator++()
		{
			CSmartValue Value;
			if (Value.Attach((void*)m_pData, m_DataLen, CSmartValue::VT_UNKNOWN))
			{
				m_pData += Value.GetDataLen();
	...
	{
		return AddMember(ID, (const unsigned char*)Data.GetBuffer(), Data.GetUsedSize());
	}
	...
	iterator begin()
	{
		return iterator(m_pData + sizeof(BYTE) + sizeof(UINT), GetLength());
	}
