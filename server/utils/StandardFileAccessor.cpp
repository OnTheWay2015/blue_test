...
UINT64 CStandardFileAccessor::GetSize()
{
	if(m_hFile)
	{
...
UINT64 CStandardFileAccessor::Read(LPVOID pBuff,UINT64 Size)
{
	return fread(pBuff,1,(size_t)Size,m_hFile);
}

UINT64 CStandardFileAccessor::Write(LPCVOID pBuff,UINT64 Size)
{
	UINT64 WriteSize=fwrite(pBuff,1,(size_t)Size,m_hFile);
	if(m_IsWriteFlush)
	{
		fflush(m_hFile);
	}
...
BOOL CStandardFileAccessor::Seek(INT64 Offset,int SeekMode)
{
...
UINT64 CStandardFileAccessor::GetCurPos()
{
