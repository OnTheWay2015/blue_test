...
/*
import { _decorator, Component, Enum, String, Node, Color, Label, assetManager, Sprite, resources, Texture2D, SpriteFrame, warn } from 'cc';
...
function _setColor(node: Node, prop: any) {
    node.color = new Color(prop.r, prop.g, prop.b);
}
...
function _setOpacity(node: Node, prop: any) {
    node.opacity = prop;
}
function _setSlew(node: Node, prop: any) {
...
function _setLabelString(node: Node, prop: any) {
    node.getComponent(Label).string = prop;
}
function _setAll(node: Node, prop: any) {
    ...
    node.color = new Color(nodeProp.color.r, nodeProp.color.g, nodeProp.color.b);
...
function _setSpriteTexture(node: Node, prop: any) {
    let url = prop.url ?? "";
    let uuid = prop.uuid ?? "";
    ...
    if(CC_EDITOR) {
        assetManager.loadAny({uuid: uuid}, (error, data) => {        
            if(error) {
                Editor.warn('PropController  load sprite texture faild', prop, error);
                return ;
            };
            node.getComponent(Sprite).spriteFrame = data;
        });
        return;
    }
    cc.resources.load<Texture2D>(url, Texture2D, (error, data: Texture2D) => {        
        if(error) {
            Editor.warn('PropController  load sprite texture faild', url, error);
            return ;
        };
        node.getComponent(cc.Sprite).spriteFrame = new SpriteFrame(data);
    });
}
const _localSetFunc: {[key: number]: (node: Node, prop: any) => void} = {};
function _regiestSetFunction(id: number, func: (node: Node, prop: any) => void) {
    if(_localSetFunc[id]) {
        warn("");
    }
...
@executeInEditMode@menu('i18n:状态控制/PropController')@inspector('packages://propcontroller/dist/inspector.js')
export default class PropController extends Component {
    ...
    private _getNodeBySiblingIndex(path: string, node: Node) {
