import { _decorator, Rect, Vec2 } from 'cc';
...
//            // 检查矩形是否符合规则 -> 即是否有重叠
    ...
    private _initRoomGrid(room: Rect) {
...
//            // 当前grid有可以扩展的方向
    ...
    private isRemove(merged: {[key: number]: number}, connectRegions: {[key: number]: number[]}, connector: Vec2, grid: Vec2) {
...
//    /** 雕刻一个格子 */
    ...
    private getGridType(grid: Vec2) {
        // return this.map[grid.y * this.width + grid.x];
    }
    private canCarve(grid: Vec2, dir: DirType) {
