...
//import { _decorator, RenderFlow, Component, Sprite, Camera, Texture2D, RenderTexture, Material, EffectAsset, matView, matViewInv, matProj, matProjInv, matViewProj, matViewProjInv, cameraPos, time, screenSize, screenScale, matWorld, matWorldIT } from 'cc';
import { _decorator, RenderFlow, Component, Sprite, Camera, Texture2D, RenderTexture, Material, EffectAsset, view, Button, Color, find, tween, UIOpacity, gfx, SpriteFrame, deserialize } from 'cc';
...
//const BAN_FALG = (RenderFlow.FLAG_RENDER | RenderFlow.FLAG_POST_RENDER);
//const BAN_FALG = (RenderFlow.FLAG_RENDER | RenderFlow.FLAG_POST_RENDER);
  ...
  "shaders": [
    {
      "hash": 3005313742,
      "glsl3": {
        "vert": "\nprecision highp float;\nuniform CCGlobal {\n  mat4 cc_matView;\n  mat4 cc_matViewInv;\n  mat4 cc_matProj;\n  mat4 cc_matProjInv;\n  mat4 matViewProj;\n  mat4 matViewProjInv;\n  vec4 cc_cameraPos;\n  vec4 cc_time;\n  mediump vec4 cc_screenSize;\n  mediump vec4 cc_screenScale;\n};\nuniform CCLocal {\n  mat4 cc_matWorld;\n  mat4 cc_matWorldIT;\n};\nin vec3 a_position;\nin vec4 a_color;\nout vec4 v_color;\n#if USE_TEXTURE\nin vec2 a_uv0;\nout vec2 v_uv0;\n#endif\nvoid main () {\n  vec4 pos = vec4(a_position, 1);\n  #if CC_USE_MODEL\n  pos = matViewProj * cc_matWorld * pos;\n  #else\n  pos = matViewProj * pos;\n  #endif\n  #if USE_TEXTURE\n  v_uv0 = a_uv0;\n  #endif\n  v_color = a_color;\n  gl_Position = pos;\n}",
...
//     */
    public init() {
         ...
         CocosHelper.NodeSetSize(this.node,size.width,size.height);
         ...
         //this.node.opacity = 0;
         CocosHelper.NodeSetAlph(this.node,0);
  ...
  public async showModal(lucenyType: number, time: number = 0.6, isEasing: boolean = true, dualBlur = false) {
    if (dualBlur) {
      if (!this.camera) {
        ...
        let node =CocosHelper.NodeCreate("BlurCamera");
        this.camera = node.addComponent(Camera);
        find('Canvas').addChild(node);
      }
      //this.useDualBlurSprite(this.camera); //todo
      CocosHelper.NodeSetColor(this.node,Color.WHITE);
      //this.node.color = Color.WHITE;
    } else {
      this.useNormalSprite(this.sprite);
      //this.node.color = cc.Color.BLACK;
      CocosHelper.NodeSetColor(this.node,Color.BLACK);
    }
    ...
    if (isEasing) {
      ...
      await CocosHelper.runTweenSync(opacityComp, tween().to(time, { opacity: o }));
    } else {
      CocosHelper.NodeSetAlph(this.node,0);
      //this.node.opacity = o;
    }
...
//    /** 代码创建一个单色texture */
  ...
  private getSingleTexture() {
    ...
    // 初始化Texture2D
    let info = new gfx.TextureInfo();
    info.format = gfx.Format.RGBA8;
    info.width = 2;
    info.height= 2;
    let x = texture.getGFXTexture();
    x.initialize(info);
    ...
    private _renderTextures: RenderTexture[] = [];
