...
class ToastMgr {
    private _pools: {[key: string]: Pool<UIToast>} =  js.createMap();
    private _showingToast: {[key: string]: Array<UIToast>} = js.createMap();
    ...
    private async genPool(finfo: IFormConfig ) {
        // 对pool创建一个独立结点
        // todo...
        let prefabUrl = finfo.prefabUrl;
        let prefab = await ResMgr.inst.loadFormPrefab(prefabUrl);
        let pool = this._pools[prefabUrl] = new Pool(() => {
            let node = instantiate(prefab);
            UIManager.getInstance().addNode(node,finfo);
