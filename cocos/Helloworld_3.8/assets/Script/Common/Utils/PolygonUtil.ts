...
//     * @param useDichotomy 
//     */
    public static splitPolygonByLine(l0: Vec2, l1: Vec2, polygon: Vec2[], useDichotomy = false) {
        // let result: number[] = [];
...
//    //返回值：[n,p] n:0相交，1在共有点，-1不相交  p:交点
    public static lineCrossPoint(p1:Vec2,p2:Vec2,q1:Vec2,q2:Vec2): [number, Vec2]{
...
//    //两条线段是否跨立
//    //即非平行
    public static isLineSegmentCross(P1:Vec2,P2:Vec2,Q1:Vec2,Q2:Vec2)
    {
        if(
...
//    // 返回值: -1:不相交, 0:相交, 1:点在线段上
    public static rayPointToLine(point:Vec2,linePA:Vec2,linePB:Vec2){
//        // 定义最小和最大的X Y轴值  
...
//    //返回值: -1:在多边形外部, 0:在多边形内部, 1:在多边形边线内, 2:跟多边形某个顶点重合
    public static relationPointToPolygon(point:Vec2,polygon:Vec2[])
    {
        let count = 0;
...
//    //返回多边形数组
//    //如果没有被切割，返回空数组
    public static lineCutPolygon(pa:Vec2,pb:Vec2,polygon:Vec2[]){
