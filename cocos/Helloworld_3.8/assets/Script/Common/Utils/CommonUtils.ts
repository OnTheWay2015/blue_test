import { _decorator, Label, Sprite, Node, Vec2,SpriteFrame,EventTouch,view,rect, UITransform, tween} from 'cc';
    ...
    public static addSimpleClick(target: Node, cb: () => void) {
        ...
        targetNode.on(Node.EventType.TOUCH_END, (e: EventTouch) => {
            if (lastTouchPos) {
                ...
                // 新写法：使用Vec2.subtract计算差值向量，然后调用length()获取长度
                //const delta = Vec2.subtract(new Vec2(), this.lastTouchPos, currentPos).length();
    ...
    static makeMaxWidthLabel(label: Label, width: number): Label {
        ...
        Object.defineProperty(obj, "string", {
            ...
            set(str) {
                ...
                if (uiTransform && uiTransform.width > width) {
                    label.overflow = Label.Overflow.RESIZE_HEIGHT; //保持文本自适应
                    //label.node.setContentSize(width, 1);
                    // 设置新尺寸
                    uiTransform.setContentSize(width, uiTransform.height);
...
//          // uv原点是左上角
    ...
    public static tweenFloat(from: number,
        to: number,
        duration: number,
        onUpdate: (t: number) => void,
        onComplete?: (target?: Record<string, number>, data?: unknown) => void,
    ...
    let o: Record<string, number> = { _value: from };
        ...
        let t= tween(o).to(duration, { value: to }).call(onComplete);
        if (autoStart) {
            t.start();
        }
        return t;
    }
    public static tweenVec2(from: Vec2, to: Vec2, duration: number, onUpdate: (t: Vec2) => void,
        onComplete?: (target?: Record<string, Vec2>, data?: unknown) => void,
        autoStart: boolean = true) {
        let o: Record<string, Vec2> = { _value: from };
        Object.defineProperty(o, 'value', {
            get: () => o._value,
            set: (v: Vec2) => { o._value = v; onUpdate && onUpdate(o._value); },
        });
        let t = tween(o).to(duration, { value: to }).call(onComplete);
        if (autoStart) {
            t.start();
...
//             set: (v: number) => { o._value = v; onUpdate && onUpdate(o._value); },
//         });
//         let tween = tween(o).to(duration, { value: to }).call(onComplete);
//         if (autoStart) {
//             tween.start();
//         }
//         return tween;
//     }
// 
//     public static tweenVec2(from: Vec2, to: Vec2, duration: number, onUpdate: (t: Vec2) => void, onComplete?: Function, autoStart: boolean = true) {
//         let o: Record<string, Vec2> = {_value: from};
//         Object.defineProperty(o, 'value', {
//             get: () => o._value,
//             set: (v: Vec2) => { o._value = v; onUpdate && onUpdate(o._value); },
