/*
====> 引入其他头文件：通常会 通过 USE_STDPERIPH_DRIVER 宏开头,引入其他与具体微控制器型号相关的头文件
			如 stm32f10x_conf.h，增强代码的可移植性


stm32f10x.h 是 STM32F10x 系列微控制器开发中的核心头文件，其作用相当于51单片机开发中的 reg52.h。
该文件提供了完整的寄存器映射和封装，是STM32标准外设库的基础。

具体来说，该头文件的主要功能包括：

内存映射定义：定义了片上外设的基地址，包括PERIPH_BASE(0x40000000)，以及基于此的APB1、APB2、AHB总线外设地址空间
	例如，GPIOA的基地址定义为APB2PERIPH_BASE + 0x0800

寄存器结构体封装：通过结构体将相关寄存器分组封装，方便以面向对象的方式访问。
	如RCC（复位和时钟控制）外设就被定义为包含CR、CFGR、CIR等多个寄存器成员的结构体类型RCC_TypeDef

外设寄存器访问宏：为每个外设定义了访问宏，例如#define RCC ((RCC_TypeDef *) RCC_BASE)，使得开发者可以直接通过RCC->CR这样的形式操作寄存器

常量和宏定义：包含与STM32F10x系列微控制器相关的各种常量和宏定义，用于设置和操作外设寄存器
 

该文件实现了从寄存器地址到结构体变量的映射，无论是寄存器开发还是库函数开发都必不可少
通过这种封装，开发者无需直接操作复杂的寄存器地址，大大简化了STM32的开发流程
