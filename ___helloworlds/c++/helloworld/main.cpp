...
void testAddress() 
{
    ...
    unsigned long long aryP1 = (unsigned long long)(&ary[0]); //取数组第一个元素的地址, 转为地址数值
    //aryP1 和  aryP  值是一样的
    unsigned long long aryP = (unsigned long long)ary; //数组名是一个指针，直接可以转为地址数值
...
extern int test_templatefactory() ;
extern int test_templatecalc() ;
