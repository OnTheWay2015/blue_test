...
typedef struct global_State {
	...
	// 一个阈值，当这个totalbytes大于这个阈值时进行自动GC
	lu_mem GCthreshold;  /* 垃圾回收触发阈值，初始化默认是 20, 当已分配内存超过该值时触发自动GC,动态赋值, 在 luaC_adjustthreshold */
	lu_mem totalbytes;  /* 当前已分配的内存字节总数     在 luaM_alloc --> luaM_realloc_  处设置变化 */
	// 一个估算值，根据这个计算GCthreshold
...
struct lua_State {
    ...
    StkId base;  /* 当前函数的栈基地址（指向当前调用函数在栈中位置的后一位. 如果有参数，就是参数起始位置, 参数是从左到右入栈） */
    ...
    TValue l_gt;  /* 全局环境表（存放全局变量的核心表，对应Lua中的 _G 全局表） */
					//默认是没有设置元表,需要用时 手动设置					
