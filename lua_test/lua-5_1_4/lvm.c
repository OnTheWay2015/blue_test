...
void luaV_execute (lua_State *L, int nexeccalls) {
 ...
 reentry:  /* entry point */
  ...
  pc = L->savedpc; //.faq  在哪赋值? Protect 时有
  ...
  for (;;) {
    ...
    lua_assert(base == L->base && L->base == L->ci->base); //.faq ?? 第一个方法？

    lua_assert(base <= L->top && L->top <= L->stack + L->stacksize);//栈大小不超
    ...
    switch (GET_OPCODE(i)) {
      ...
      case OP_LOADK: { //从 Proto->k 表里按索引 KBx(i), 取值到栈上(ra)
//#define KBx(i)	check_exp(getBMode(GET_OPCODE(i)) == OpArgK, k+GETARG_Bx(i))
        setobj2s(L, ra, KBx(i));
        continue;
      }
      ...
      case OP_SETGLOBAL: {
        TValue g;
        sethvalue(L, &g, cl->env); //cl->env 默认是全局表
      ...
      case OP_RETURN: {
        int b = GETARG_B(i);
        if (b != 0) L->top = ra+b-1;  // .faq OP_TAILCALL时为什么不用 -1?
